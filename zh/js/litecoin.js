window.addEventListener("load", () => {
    ...
    document.getElementById('litecoin_configure_network').addEventListener('change', (ev) => {
        ...
        document.getElementById('litecoin_new_path').parentNode.querySelector('span').innerText = `　（说明：路径m/84'/${ev.target.value == 0 ? 2: 1}'/i'/0/j表示第i+1个账户里的第j+1个钱包，i,j=0、1、2、…）`;
    })
    ...
    document.getElementById('litecoin_fromPrivateKey_ok_btn').addEventListener('click', async (et) => {
        ...
        try {
            ...
            let keyPair = ECPair.fromWIF(pri_key, litecoin_network);//压缩私钥产生压锁公钥，非压缩私钥产生非压缩公钥。
            ...
            document.getElementById('litecoin_address_td1').innerHTML = `P2TR地址：<br>P2WPKH地址：<br>P2SH-P2WPKH：<br>P2PKH地址：<br>`;
            document.getElementById('litecoin_address_td2').innerHTML = `${address_p2tr}（密钥路径）<br>${address_p2wpkh}<br>${address_p2sh_p2wpkh}<br>${address_p2pkh}`;
            ...
            document.getElementById('litecoin_private_td1').innerHTML = `压缩WIF：<br>非压缩WIF：<br>Base58编码：<br>原始（十六进制）：<br>原始（二进制）：`;
            ...
            if (keyPair.compressed) {
                ...
                pub_uncompress = ECPair.fromWIF(pri_uncompress, litecoin_network).publicKey.reduce((str, byte) => str + byte.toString(16).padStart(2, '0'), '');//非压缩私钥产生非压缩公钥
            }
            document.getElementById('litecoin_public_td1').innerHTML = `压缩公钥：<br>非压缩公钥：<br>`;
            document.getElementById('litecoin_public_td2').innerHTML = `${pub_compress}（ECDSA算法产生的公钥-最普遍）<br>${pub_uncompress}`;
    ...
    document.getElementById('litecoin_wallet_balance_btn').addEventListener('click', async (et) => {
        ...
        if (wallet_address.length < 64) {//按钱包地址查询
            ...
            litecoin_getUtxo(wallet_address, litecoin_network == litecoinTestnet).then((utxoes) => {
                ...
                document.getElementById('litecoin_view_wallet_balance').innerHTML = `余额：${balance} Litoshi（${balance / 100000000} LTC）。共有${utxoes.length}个UTXO：`
                let ls = '';
                let ms = '';
                let rs = '';
                //                let i = 1;
    ...
    /*
        document.getElementById('litecoin_fee_dollars').addEventListener('input', (ev) => {
            //        rate = parseFloat(document.getElementById('rate').value.replace(/,/g, ''));
            fee = Math.round(ev.target.value / litecoin_rate * 100000000);
            document.getElementById('litecoin_fee').innerText = fee;
            let toOut = document.getElementById('litecoin_tx_itInput').innerText.replace(/,/g, '');
            let totalAmount = document.getElementById('litecoin_tx_amount').innerText.replace(/,/g, '');
            let tx_fee = parseInt(totalAmount) - parseInt(toOut);
    ...
    */
    ...
    document.getElementById('litecoin_tx_me_utxo').addEventListener('click', async (et) => {
        ...
        if (canFetchRawTX) {
            ...
            litecoin_getUtxo(wallet_address, litecoin_network == litecoinTestnet).then((utxoes) => {
                let balance = utxoes.reduce((totalAmount, e) => totalAmount + e.value, 0);
                document.getElementById('tx_wallet_balance').innerHTML = `<br>钱包余额：${balance} Litoshi（${balance / 100000000}个莱特币），共有${utxoes.length}个UTXO：`
                let ms = '';
                let rs = '';
                //            let i = 0;
                outx = utxoes;
                for (let i = 0; i < utxoes.length; i++) {
                    ...
                    rs = rs + `<div data-uid='${uid}'>&nbsp;<a href="javascript:view_tx('${b.txid}')" style="text-decoration:none">${b.txid}</a><br>&nbsp;${b.vout}<br>&nbsp;${new Intl.NumberFormat('en-US').format(b.value)} Litoshi<button data-i=${i} class="litecoin_tx_btn_addInput" style="float: right;">花费</button><span style="visibility: hidden;float: right;color: green;font-size: 1.5rem;margin-right: 10px;">√</span><br><hr></div>`;
                    //                i++;
                    //                if (i > 5) break;
                }
                ...
                document.querySelectorAll(".litecoin_tx_btn_addInput")?.forEach(btn => {
                    ...
                    <span>序号：</span><b class='output_index'>${outx[i].vout}</b><br><span>金额：</span><b class='value'>${new Intl.NumberFormat('en-US').format(outx[i].value)}</b> Litoshi<br>
    ...
    document.getElementById('litecoin_output_tx_hex').addEventListener('click', (ev) => {
        ...
        document.getElementById('dec_fee').innerHTML = `交易Id：${tx.getId()}<span style="float: right">交易费用：${tx_fee} Litoshi（${document.getElementById('litecoin_tx_fee_dollar').innerHTML}美元）</span>`;
    });
