...
window.addEventListener("load", () => {
    document.getElementById('solcoin_configure_network').addEventListener('change', (ev) => {
        ...
        document.getElementById('solcoin_new_path').parentNode.querySelector('span').innerText = `　（说明：路径m/44'/${solcoin_isTestNet ? 1 : 501}'/i'/j'表示第i+1个账户里的第j+1个钱包，i,j=0、1、2、…、2147483647）`;
    ...
    document.getElementById('solcoin_wallet_balance_btn').addEventListener('click', async (evt) => {
        ...
        if (addressORtxid.length == 44) {//按钱包地址查询，如：66TrHUCZLTyiRHAhLBE6Wwr3NPccd5KS3EFQGFWhwgDd
            ...
            connection.getBalance(publicKey).then(balance => {
                document.getElementById('solcoin_view_wallet_balance').innerHTML = `钱包余额（索拉纳）：${balance / solanaWeb3.LAMPORTS_PER_SOL} SOL`;
                closeModal();
            ...
            //    maxSupportedTransactionVersion: 0, // 建议加上，避免版本不兼容
            //});
    ...
    document.getElementById('solcoin_generate_sign_btn').addEventListener('click', (ev) => {
        ...
        //            const keypairBytes = bs58check.default.decode(secretKey);
        //            const keypair = solanaWeb3.Keypair.fromSecretKey(keypairBytes);
        /*        if (secretKey.length < 60) {//私钥
                    const password = document.getElementById('solcoin_sign_password').value.trim();
                    if (password != '') {//需要解密私钥
                        ...
                        try {
                            ...
                            privateKey = Buffer.Buffer.from(decryptKey.privateKey).toString('hex');
                        } catch (error) {
                            alert(`解密私钥出错：${error}`);
                            return;
                        }
                    ...
                    const keypair = solanaWeb3.Keypair.fromSeed(Buffer.Buffer.from(privateKey, 'hex'));
    ...
    document.getElementById('solcoin_ok_transfer').addEventListener('click', async function f(evt) {
        ...
        /*        
                const password = document.getElementById('solcoin_private_password').value.trim();
                if (password != '') {//需要解密私钥
                    let N = parseInt(document.getElementById('N_id').value.trim());
                    let r = parseInt(document.getElementById('r_id').value.trim());
                    let p = parseInt(document.getElementById('p_id').value.trim());
        ...
        */
        ...
        //        const fromWallet = solanaWeb3.Keypair.fromSeed(Buffer.Buffer.from(privateKey, 'hex'));
        if (data != '') {//在链上保存数据资料
        ...
        try {
            ...
            //const txHex = Buffer.Buffer.from(serializedTx).toString("hex");//转成 HEX
            const txBase64 = Buffer.Buffer.from(serializedTx).toString("base64");//转成 base64
            document.getElementById('solcoin_result_transfer').innerHTML = `
                程序运行错误：${err}<br>
                可把下面被序列化的交易拷贝到https://www.walletoffline.com/crypto或https://explorer.solana.com/tx/submit上发布（注意：必须在一分钟内发布，否则就过期了）：<br>
            ...
            document.getElementById('solcoin_result_transfer').querySelector('textarea').addEventListener("focus", function (ev) {
                // 延迟一下，避免某些浏览器在 focus 时 select 被覆盖
                setTimeout(() => {
                    ev.target.select();
                }, 1);
            });
    ...
    document.getElementById('solcoin_dispatch_tx').addEventListener('click', async (ev) => {
        ...
        try {
            ...
            document.getElementById('solcoin_dispatch_result').innerHTML = `交易已发送，签名ID：${signature}<br>查看交易: https://explorer.solana.com/tx/${signature}?cluster=${solcoin_network}`;
        } catch (err) {
            document.getElementById('solcoin_dispatch_result').innerHTML = `程序运行错误：${err}<br>`;
        }
...
function solcoin_recover_wallet() {
    ...
    if (!bip39.validateMnemonic(hd_wallet.mnemonic, solcoin_language) && hd_wallet.secretKey == '') {
        alert("至少输入一个合法的助记词或者一个合法的密钥");
        return;
    }
    if (hd_wallet.secretKey != '') {//从密钥恢复钱包
        //        const keypairBytes = bs58check.default.decode(hd_wallet.secretKey);
        //        const keypair = solanaWeb3.Keypair.fromSecretKey(keypairBytes);
        /*        if (hd_wallet.password != '') {//需要解密私钥
                    let N = parseInt(document.getElementById('N_id').value.trim());
                    let r = parseInt(document.getElementById('r_id').value.trim());
                    let p = parseInt(document.getElementById('p_id').value.trim());
                    try {
                        let decryptKey = bip38.decrypt(hd_wallet.privateKey, hd_wallet.password, null, { N: N, r: r, p: p });
                        hd_wallet.privateKey = '0x' + Buffer.Buffer.from(decryptKey.privateKey).toString('hex');
                    } catch (error) {
                        alert(`Decrypting private key failed:${error}`);
                        return;
                    }
                }
                const keypair = solanaWeb3.Keypair.fromSeed(Buffer.Buffer.from(hd_wallet.privateKey.slice(2), 'hex'));
        */
        ...
        hd_wallet.address = keypair.publicKey.toBase58();//Base58编码的32字节地址
        /*        
                hd_wallet.privateKey = '0x' + Buffer.Buffer.from(hd.derive(hd_wallet.path).privateKey).toString('hex');
                const passwd = document.getElementById('solcoin_new_password').value.trim();
                if (passwd != '') {//需要加密私钥
                    let N = parseInt(document.getElementById('N_id').value.trim());
                    let r = parseInt(document.getElementById('r_id').value.trim());
                    let p = parseInt(document.getElementById('p_id').value.trim());
                    ...
                    try {
                        hd_wallet.privateKey = bip38.encrypt(privateKey, true, passwd, null, { N: N, r: r, p: p });
                        hd_wallet.privateKey += "（私钥保护密码：" + passwd + "）";
                    } catch (error) {
                        passwd = '';
    ...
    document.getElementById('solcoin_hd_wallet').innerHTML = `
    　　　　助记词：${hd_wallet.mnemonic}<br>
    助记词保护密码：${hd_wallet.mnemonic_passwd}<br>
    　　　　　密钥：${hd_wallet.secretKey}<br>
    　　　　　地址：${hd_wallet.address}（也是公钥）<br>
    　　　　　路径：${hd_wallet.path}
    `;
}